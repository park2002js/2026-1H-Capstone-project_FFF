...
namespace FFF.Battle.Modifier
{
    ...
    public class HandJokboCondition : IModifierCondition
    {
        ...
        public bool IsMet(ModifierContext context)
        {
            ...
            {
                HandCategory.Ddaeng =>
                    hand >= SeotdaHand.SamPalGwangDdaeng && hand <= SeotdaHand.IlDdaeng,
                HandCategory.Kkeut =>
                    (hand >= SeotdaHand.GuKkeut && hand <= SeotdaHand.IlKkeut) ||
                    hand == SeotdaHand.MangTong,
                HandCategory.Special =>
                    hand >= SeotdaHand.Ali && hand <= SeotdaHand.JangSa,
                _ => false
            };
        }
    }
